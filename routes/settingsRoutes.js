...
module.exports = (app) => {
  ...
  app.get('/api/waterAreas', async (req, res) => {
    conn('select * from syswaterarea', (err, result) => {
      if (err) {
        res.send({
          data: null,
          meta: {
  ...
  app.post('/api/waterArea', (req, res) => {
    const { province, city, district, areaCode, remark } = req.body
    try {
      conn(
        'insert into syswaterarea (??,??,??,??,??) values(?,?,?,?,?);',
        [
          'AreaCode',
          'Province',
          'City',
  ...
  app.delete('/api/waterArea/:id', (req, res) => {
    const { id } = req.params
    conn('DELETE FROM syswaterarea WHERE ??=?', ['id', id], (err, result) => {
      if (err) {
        res.send({
          data: null,
          meta: {
